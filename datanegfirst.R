#用于NegGOA与数据的处理的整合，此处为第一部分，为NegGOA 提供必要的数据
work.path="H://R//CODES"
data.path="H://R//DATA"
...
#生成各节点的层级深度，此深度与之前用的定义不同，根结点为深度1
...
neggoa.result=NegGoaSelection(select.node=select.node.3,graph.BP.level.50,match.go.table,common.genes)

setwd("H://R//DATA//0220")
data.total=BuildTrainNegGOA(neggoa.result, except.root.labels.3, remain.data,
                            adjust.ratio=0.2,select.attributes.en=FALSE,write.en=TRUE)
