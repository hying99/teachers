...
Ytrain.labels=root.table.3[,20]
Ytest.labels=test.select.table[,20]
model1 <- rirls.spls(Xtrain=Xtrain, Ytrain=Ytrain.labels, lambda.ridge=2, lambda.l1=0.5, ncomp=76,
                     Xtest=Xtest, adapt=TRUE, maxIter=100, svd.decompose=TRUE)
predict.labels[,20]=model1$hatYtest
sum(model1$hatYtest!=Ytest.labels) / length(Ytest.labels)

for(j in 1:label.num)
{
  Ytrain.labels=root.table.3[,j]
  Ytest.labels=test.select.table[,j]
  model1 <- rirls.spls(Xtrain=Xtrain, Ytrain=Ytrain.labels, lambda.ridge=20, lambda.l1=0.5, ncomp=2,
                       Xtest=Xtest, adapt=TRUE, maxIter=100, svd.decompose=TRUE)
  predict.labels[,j]=model1$hatYtest
  sum(model1$hatYtest!=Ytest.labels) / length(Ytest.labels)
}
...
for(j in 1:label.num)
{
  ...
  } else
  {
    ...
    each.recall[j]=tp.num/sum(test.select.table[,j])
  }
