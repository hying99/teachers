...
ReadPredictArff<-function (data.path,fold.name,file.prefix,part.num=3)
{
  #设置数据路径
  setwd(data.path)
  #得到待读入的文件名
  ...
  original.plabels=all.labels[,(label.num+2):(label.num*2+1)]
  #各样本的pruned model得到的标签
  pruned.plabels=all.labels[,(label.num*2+3):(data.col.num-1)]
  #将标签值输出
  labels.result=list(true.labels=true.labels,original.plabels=original.plabels,pruned.plabels=pruned.plabels)
