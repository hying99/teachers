...
####由于函数采用采样方法重新生成三种数据集，故每次生成结果均不相同
                          ...
                          write.data.enable=TRUE ,row.names.enable=TRUE)
{
  ...
  valid.original=ReadData(original.valid.file,factor.col = factor.col)#读入valid基因特征属性
  valid.original.data=valid.original[[1]]#valid基因的数据信息
  #读入测试集
  original.test.file=paste("originaldata//",file.prefix,"0.propertest",sep = "")
  test.original=ReadData(original.test.file,factor.col = factor.col)#读入test基因特征属性
