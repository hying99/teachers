####由于目前训练�? 验证�? 测试集数据分配产生的差异性很大，采用此函数重新生成三种数据集 20170328
                          ...
                          write.data.enable=TRUE ,row.names.enable=TRUE)
{
  ...
  train.original.data=train.original[[1]]#基因的数据信�?
  #读入验证�?
  original.valid.file=paste("originaldata//",file.prefix,"0.valid",sep = "")
  valid.original=ReadData(original.valid.file,factor.col = factor.col)#读入valid基因特征属�?
  valid.original.data=valid.original[[1]]#valid基因的数据信�?
  #读入测试�?
  original.test.file=paste("originaldata//",file.prefix,"0.propertest",sep = "")
  test.original=ReadData(original.test.file,factor.col = factor.col)#读入test基因特征属�?
  test.original.data=test.original[[1]]#test基因的数据信�?
  #将三个数据集合并
  all.data=rbind(train.original.data,valid.original.data,test.original.data)
  #得到合并后的数据集有注释信息的基因名称列�?
